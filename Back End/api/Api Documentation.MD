...
Note that the [Refresh Login](#refresh_login) endpoint doesn't need the user's token but the user's refresh token.
...
The token is optional. It may be provided to show that the user doing the request is following the other user's profile. This allow to retrieve followers only properties (in case of a private account) which are : 
