...
This document provides a full documentation of the database used in the Bestagram project back end side.
...
Tables name must follow the *CamelCase* convention. 
...
| token                   | varchar(200)  | YES  | UNI | NULL    |                |
| refresh_token           | varchar(200)  | NO   | UNI | NULL    |                |
| token_registration_date | datetime      | YES  |     | NULL    |                |
| caption                 | varchar(1000) | YES  |     | NULL    |                |
| use_default_picture     | tinyint(1)    | YES  |     | 1       |                |
### Id
...
### Name
The name is the user's name in real life. It is chosen by the user, so he can theoretically put anything, but it is supposed to be the user's name in real life (name + surname). When **search**  queries are made, this is what is searched for. It is not unique as multiple person can have the same name.
### Hash
...
User's email.
### Public_profile
Bool column that indicates if the user wants his profile private or public.
### Token
When the user sends the correct hash as login information, a token is generated (if there is not already one and if it hasn't expired yet) and then sent back to the user. This token is then used as a key for most of the API request. For more information see [*Api Documentation.md*](<../api/Api\ Documentation.MD>).
### Refresh_token
Special token that never expires. It is used to retrieve the token of a user.
...
### Use_default_picture
Bool, wether or not the user has a custom profile picture.
## Post
